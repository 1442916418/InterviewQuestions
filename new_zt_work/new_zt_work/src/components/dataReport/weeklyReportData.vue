    ...
    <div class="monthlyReport-header">
      <div class="header-left">
        ...
        <DatePicker
          ...
          format="yyyy-MM-dd"
          type="datetimerange"
          :placeholder="$t('tip.pleaseSelectTime')"
          style="width: 200px"
        ></DatePicker>
      </div>
...
export default {
  ...
  methods: {
    initData() {
      ...
      this.ENDDATE = Number(e);
      this.WATERTYPE = w;

      let f = `${this.$moment().format("YYYY-MM-DD")} - ${this.$moment()
        .subtract(6, "d")
        .format("YYYY-MM-DD")}`;
    ...
    changeDatePicker(value) {
      ...
      this.startDate = this.$moment(value[0]).unix();
      this.endDate = this.$moment(value[1]).unix();
    },
