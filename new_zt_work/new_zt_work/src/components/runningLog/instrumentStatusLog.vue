...
<template>
  <div class="instrumentStatusLog-box">
    <div class="instrumentStatusLog-header">
    ...
    <div class="instrumentStatusLog-body">
      <DefaultTable
        :isPage="true"
        :isSelection="false"
...
import rs from "@/plugin/staticData/runingState.json";

export default {
  name: "instrumentStatusLog",
  data() {
    return {
      queryData: {
        mn: null,
    ...
    setTable() {
      ...
      if (d.length === 0) {
        ...
        for (let item of d) {
          for (let k in item) {
            if (k === "Log-Key") {
              let lo = this.setMeterName(item[k]);
            ...
            if (k.charAt(0) === "w") {
              ...
              for (let z in item[k]) {
                let r = this.setRuningState(item[k][z]);
...
.instrumentStatusLog-box {
  .single-page-container;

  .instrumentStatusLog-header {
    .header;
